...
impl Grid {
    fn adjecent_of(&self, pos: usize) -> impl Iterator<Item = usize> {
        [
    ...
    fn left_of(&self, pos: usize) -> Option<usize> {
    ...
    fn right_of(&self, pos: usize) -> Option<usize> {
        let row = self.row_of(pos);
        let new = pos + 1;
    ...
    fn top_of(&self, pos: usize) -> Option<usize> {
        pos.checked_sub(self.width)
    }

    fn bottom_of(&self, pos: usize) -> Option<usize> {
        let new = pos + self.width;
        (new < self.data.len()).then_some(new)
    }

    fn top_left_of(&self, pos: usize) -> Option<usize> {
        let new = self.top_of(pos)?;
        self.left_of(new)
    }

    fn top_right_of(&self, pos: usize) -> Option<usize> {
        let new = self.top_of(pos)?;
        self.right_of(new)
    }

    fn bottom_left_of(&self, pos: usize) -> Option<usize> {
        let new = self.bottom_of(pos)?;
        self.left_of(new)
    }

    fn bottom_right_of(&self, pos: usize) -> Option<usize> {
        let new = self.bottom_of(pos)?;
        self.right_of(new)
    }
}
